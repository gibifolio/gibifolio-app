...
class GibifolioSupabaseUser extends BaseAuthUser {
  GibifolioSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> gibifolioSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = GibifolioSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
