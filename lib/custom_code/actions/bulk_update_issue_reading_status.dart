...
Future<bool> bulkUpdateIssueReadingStatus(
  ...
  final allHaveStatus = issueIds.every((id) => readingMap[id] == statusReading);
  final String? finalStatus = allHaveStatus ? null : statusReading;

  final records = issueIds
      .map((id) => {
            'user_id': user.id,
            'issue_id': id,
            'status_ownership': ownershipMap[id],
            'status_reading': finalStatus,
